function [As, Ad, Asn] = Helm3dLocCorr(s,ka,ord)
% HELM3DLOCCORR. Helmholtz surface potentials Nystrom 
% local-corrections for the 2-periodic trapezoidal discretization 
% of a toroidal surface.
%
% [As, Ad, Asn] = Helm3dLocCorr(s,ka,m)    

if ord <= 1
    [As, Asn] = Helm3dSLPmat(s,s,ka);
    Ad = Helm3dDLPmat(s,s,ka);
elseif ord <= 3
    [As,Ad,Asn] = CorrTrap1pt(s,ka);
elseif ord <= 5
    ...
    % Corrected quadr
    [As,Ad,Asn] = CorrTrap9pt(s,ka,sub);
else
    error('ord: order of correction cannot bigger than 5')
end
end

function [As,Ad,Asn] = CorrTrap1pt(s,ka)
...
% correction
hc = 1/sqrt(s.Nv*s.Nu)/2; % h/4/pi = 1/sqrt(nv*nu)/2
h = 4*pi*hc;
N = size(As,1);
ind = sub2ind([N,N], 1:N, 1:N);
As(ind)  = (Cs0+1i*ka*h).*(s.sp*hc);
Ad(ind)  = Cd0*hc;
Asn(ind) = Cd0*hc;
end

function [As,Ad,Asn] = CorrTrap9pt(s,ka,sub)
...
[As, Asn] = Helm3dSLPmat(s,s,ka,if_jac);
Ad = Helm3dDLPmat(s,s,ka);

% correction
N = size(s.x,2);
h = 2*pi/sqrt(s.Nv*s.Nu);
hc = h/4/pi;
jac = s.sp(:);
kh = ka*h;
Cs0 = Cs0 + 1i*kh + (kh)^2/2*Cs02;
Cd0 = Cd0 + (kh)^2/2*Cd02;
...
% construct \square^{(1)}, \square^{(2)} & \square^{(3)} operators
...
% D0
[Cs02,Cd02,~,~,~,Cs0,Cd0] = epstein_zeta(s,E,F,G,Lb2(1,:)*Efac,Lb2(2,:)/2,Lb2(3,:)*Gfac);
Cs0 = -Cs0(:); Cd0 = Cd0(:);                            % O(h) errs
Cs02 = Cs02(:); Cd02 = -Cd02(:);                        % Helm O(h^3) extra
